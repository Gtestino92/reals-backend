...
- `06 Google Auth`: optional minimal Google-origin smoke flow. It requires an externally obtained Google OAuth ID token, then exchanges it through Firebase `accounts:signInWithIdp` for `firebase_google_id_token`; alternatively paste an already obtained Firebase ID token whose `firebase.sign_in_provider` is `google.com`. It does not automate OAuth and must not use Firebase password sign-in.
