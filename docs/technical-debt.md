...
- Clear current Trivy image-scan findings by updating the Spring Boot managed dependency set. The current image is based on Spring Boot 3.5.3 and Trivy flags fixed critical CVEs in `tomcat-embed-core` 10.1.42 and `spring-security-web` 6.5.1. Prefer a Spring Boot 3.5.x patch that manages fixed versions; if Tomcat fixed versions are not yet managed by Spring Boot, use a short-lived `tomcat.version` override and remove it when Spring Boot catches up.
