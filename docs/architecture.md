...
`ScheduledSecondChatStartJob` makes the second chat visible as `AVAILABLE` when the agreed start time is inside the early-entry tolerance window. It must not create a stale chat after the writable window has already expired.

`SecondChatLifecycleJob` owns second-chat lifecycle cleanup after scheduling confirmation: it closes expired scheduled windows that never created a chat, closes expired `AVAILABLE` chats that nobody entered, moves timed-out active second chats to read-only `EXPIRED`, then closes them and their connection after read-only retention.
