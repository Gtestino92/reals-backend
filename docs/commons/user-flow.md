...
- `APPROVED` is backend-authoritative and requires `now >= chat.startedAt + chat.first-chat.approval.min-elapsed-minutes`, at least `chat.first-chat.approval.min-messages-per-user` confirmed messages from the approving user and at least that many confirmed messages from the counterpart. Defaults are 5 minutes and 2 messages per participant. Too-early approval returns `FIRST_CHAT_APPROVAL_TOO_EARLY`; insufficient bilateral participation returns `FIRST_CHAT_APPROVAL_PARTICIPATION_REQUIRED`. `REJECTED`, safety cancellation and cancellation paths do not require these approval prerequisites.
