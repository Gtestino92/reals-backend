...
after that window do not initiate new start notifications. Users who already
joined are recorded as handled and are not sent to. Opening the notification
lands on Home, whose second-chat `nextSteps` are ordered by current/available
chats first, then nearest scheduled future starts, then scheduling work, then
read-only prior chats.
