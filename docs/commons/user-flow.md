...
Admins access `/api/admin/safety-reports` with `ROLE_ADMIN`. Dismissing a pending report stores review metadata and creates no penalty. Confirming a pending report creates either a temporary or permanent administrative account ban for the reported user, links it through `sourceReportId`/`penaltyId`, removes the reported user from the matchmaking queue if present and blocks normal authenticated Reals access while the ban is effective. Temporary bans stop being effective at exact `now >= expiresAt`, even before scheduler cleanup marks the row inactive.
