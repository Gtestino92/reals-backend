...
Temporary penalties require positive `durationHours`; permanent penalties reject `durationHours` and have `expiresAt = null`. Confirming a safety report is the sanction point: report creation and dismissal do not ban the reported user. Effective account bans block authenticated Reals access and defensively block matchmaking; creating a penalty removes the user from the queue if already queued. Multiple active rows are resolved as permanent winning over temporary, otherwise the latest temporary `expiresAt` wins. The penalty expiration job only normalizes expired temporary rows to inactive; it is not the authority for access at the temporal boundary.
