...
Administrative penalties are account bans; a confirmed safety report that applies a temporary ban also records an internal negative reliability event against the reported user. After a valid Firebase token resolves to an `ACTIVE` backend user, the backend rejects an effective temporary ban with `403 ACCOUNT_TEMPORARILY_BANNED` and top-level `expiresAt`, or an effective permanent ban with `403 ACCOUNT_PERMANENTLY_BANNED` and no expiry. Temporary bans are effective only while `active=true` and `now < expiresAt`; at exact `now >= expiresAt`, access is allowed even if the cleanup job has not yet marked the row inactive. A permanent ban is effective while `active=true` and `expiresAt=null`. Permanent-ban creation fully contains active engagements. Temporary-ban creation always removes the user from matchmaking and selectively contains only engagements whose actionable window is not viable after the effective expiry plus the configured resume margin.
...
face presence only as an MVP person-photo signal. DEV rejects `sightengine` and
uses the provider `none` compatibility path, so it makes no Sightengine calls.
At least one `faces` entry sets
...
suggestive, violence/threat, gore and hate/extremism signals. Sexual suggestive
uses Sightengine `very_suggestive`, `suggestive` and concerning
`suggestive_classes` scores such as `visibly_undressed`, `lingerie`,
`suggestive_pose`, `suggestive_focus`, `sextoy` and `nudity_art`; it ignores
`mildly_suggestive` and ordinary swimwear-like classes such as `bikini`.
Reject thresholds produce `REJECTED`, review thresholds produce `NEEDS_REVIEW`,
and otherwise moderation is `APPROVED`. The default sexual-suggestive review
threshold is `0.50` and has no automatic reject threshold. `NEEDS_REVIEW`
enters the existing admin review queue. Automatic provider moderation does not
create safety reports, child-safety
...
- `GET /api/matches/{matchId}/visual-profile`: fetch partner profile only while visual content is available. During `VISUAL_PHASE`, the visual review must exist, server time must be greater than or equal to its availability timestamp, and its `visualExpiresAt` deadline must still be in the future by server time. After `VISUAL_APPROVED`, a non-closed connection for the same match must exist and include the requester. Blocked pairs, unrelated users, `CHAT_ACTIVE`, `CHAT_REJECTED`, `VISUAL_REJECTED` and `EXPIRED` matches are denied. The response includes only partner photos whose `moderationStatus=APPROVED`, with freshly generated read URLs only for those included photos. `NEEDS_REVIEW` and `REJECTED` profile photos remain visible to owner/admin paths but are not externally visible here. The response also includes `visualExpiresAt`, `myPersonalMessageSubmitted`, partner personal-message submitted/read flags for client emphasis, `decisionRequiresPartnerPersonalMessageRead` retained temporarily as always `false`, and required `affinityIndicators: [] | [{ categoryId, title }]` containing at most three positive shared category snapshots.
- `POST /api/matches/{matchId}/chat-decision`: submit first-chat continuation decision. `APPROVED` is individual and requires both users to move the match to `VISUAL_PHASE`; before persisting `APPROVED`, the backend requires the configured minimum elapsed first-chat time and configured minimum confirmed messages from both participants. Local and dev profiles default to no approval minimums (`0` minutes and `0` messages), the shared default remains 1 minute and 3 messages per participant, and production explicitly defaults to 3 minutes and 3 messages per participant. Too-early approval returns `FIRST_CHAT_APPROVAL_TOO_EARLY`; insufficient bilateral participation returns `FIRST_CHAT_APPROVAL_PARTICIPATION_REQUIRED`. `REJECTED` remains available without those approval prerequisites. `REJECTED` while the counterpart is still pending is unilateral cancellation: it closes the first chat, moves the match to `CHAT_REJECTED`, releases locks and records first-chat unilateral-close reliability when applicable; it does not create an account ban. `REJECTED` after the counterpart already approved persists the final decision pair, finishes the chat as `FIRST_CHAT_DECISION_MISMATCH`, moves the match to `CHAT_REJECTED`, releases locks and does not create cancellation artifacts, penalties, mutual-no-spark reliability rewards or unilateral-close reliability events. Once one participant has approved and the other remains pending, only the pending participant may submit the remaining final decision. If the first-chat deadline already passed, the backend rejects with `CHAT_EXPIRED`; if the inactivity deadline already passed outside decision-only state, it rejects with `CHAT_ABANDONED`.
...
- `POST /api/chats/{chatId}/messages`: send a TEXT message as authenticated user. The request accepts legacy `{ "content": "..." }` and additive `clientMessageId` plus nullable `replyTo: { type, targetId }`. A quoted reply requires `clientMessageId`; exact TEXT replay for the same `(chat, sender, clientMessageId)`, normalized content and direct reply target returns the original message before ordinary mutable eligibility checks. Reusing the key with different content, target or message type returns `CHAT_MESSAGE_IDEMPOTENCY_CONFLICT`. `replyTo.type = MESSAGE` may target only the other participant's TEXT or AUDIO message in the same chat. `replyTo.type = GUIDANCE_QUESTION` may target any same-chat `ConversationPromptSnapshot` in a first chat. Invalid, missing or wrong-chat targets return opaque `CHAT_MESSAGE_REPLY_TARGET_NOT_AVAILABLE`. First-chat sends after absolute timeout are rejected with `CHAT_EXPIRED`; sends after inactivity timeout are rejected with `CHAT_ABANDONED`. While a `MUTUAL_CANCEL` exit request is `PENDING`, message sends are rejected with `CHAT_MUTUAL_CANCELLATION_PENDING`. In first-chat decision-only state, ordinary sends are rejected with `FIRST_CHAT_DECISION_ONLY`.
- `POST /api/chats/{chatId}/audio-messages`: send an AUDIO message with multipart `file` and required client-generated UUID `clientMessageId`. Optional shallow reply metadata uses `replyToType` plus `replyToTargetId`; both fields must be present together or both omitted. `replyToType` accepts `MESSAGE` or `GUIDANCE_QUESTION`, and target availability follows the same rules as TEXT replies. `clientMessageId`, `replyToType` and `replyToTargetId` are scalar `text/plain` multipart parts; `file` remains the binary audio part. Malformed or partial reply metadata returns `400 CHAT_MESSAGE_INVALID`; a structurally valid but unavailable target returns `409 CHAT_MESSAGE_REPLY_TARGET_NOT_AVAILABLE`. New logical messages return `201 Created`. An exact replay in the shared `(chat, sender, clientMessageId)` namespace with the same accepted audio SHA-256 and the same direct reply target returns the original canonical message with `200 OK` before ordinary mutable eligibility checks. Reusing the key with different audio bytes, direct reply target or message type returns `409 CHAT_MESSAGE_IDEMPOTENCY_CONFLICT`. In first-chat decision-only state, new audio sends are rejected with `FIRST_CHAT_DECISION_ONLY`.
- `PUT /api/chats/{chatId}/messages/{messageId}/reaction`: persist an irreversible `HEART` reaction on one message sent by the other participant. `HEART` is currently the only supported reaction type, a message can have at most one reaction, and reactions cannot be removed or toggled. A new reaction is allowed only for messages in the caller's latest incoming block: messages from the other participant after the caller's most recent own message strictly before the latest incoming message, through that latest incoming message, ordered by `(sentAt, id)`. Own messages sent after that latest incoming block do not invalidate it; a newer incoming block supersedes a previous unreacted block. Repeating `PUT HEART` on an already-hearted message returns the canonical message successfully even after the message has left the current block or the chat later becomes read-only. New reactions are metadata only: they do not count as conversational activity, update `sentAt`, reset inactivity, extend deadlines, affect reliability/penalties, trigger notifications, alter scheduling/Home state or create chat-message side effects.
...
- `POST /api/chats/{chatId}/exit-requests/{exitRequestId}/rejection`: reject mutual cancellation and close the chat without penalty.
- `POST /api/chats/{chatId}/exit-requests/{exitRequestId}/timeout`: resolve an unanswered mutual cancellation after the configured timeout and close the chat. If the requester calls this because the responder did not answer in time, the requester is not penalized.
- `POST /api/chats/{chatId}/cancellations`: unilateral cancellation. Closes the first chat and records first-chat unilateral-close reliability when applicable; ordinary cancellation does not create an account ban. In first-chat decision-only state, direct ordinary cancellation is rejected with `FIRST_CHAT_DECISION_ONLY`; submit final `REJECTED`, safety report or manual block instead.
- `POST /api/chats/{chatId}/safety-cancellations`: safety/report cancellation. Requires non-blank `details`, closes the chat with `ChatEndReason.SAFETY_REPORT`, creates an internal `SafetyReport` in `PENDING` status and returns `penaltyApplied=false`. Optional `blockUser: true` additionally creates or reuses a directional `UserBlock` from reporter to reported with `source = SAFETY_REPORT`; omitted or `false` does not create a permanent block. `ChatExitReason.CHILD_SAFETY_CONCERN` maps explicitly to `SafetyReportReason.CHILD_SAFETY_CONCERN`. Reporting does not automatically penalize or ban the reported participant; penalties are applied only after admin/backoffice review.

Audio messages are part of the same ordered chat-message stream as text, not a separate feed. TEXT and AUDIO share the same `(chat, sender, clientMessageId)` idempotency namespace and the same direct reply-target model. Message DTOs add nullable `clientMessageId`, `messageType` (`TEXT` or `AUDIO`), `audio`, nullable `reactionType` (`HEART` or `null`) and nullable `replyTo`. Text messages keep non-null `content` and `audio = null`; audio messages have `content = null` and `audio = { url, durationMillis, contentType, sizeBytes }`. `replyTo` is a server-derived shallow preview of exactly one direct target; it never includes nested quote chains, target reactions or full target DTOs. MESSAGE previews expose `type`, `targetId`, `senderId`, `messageType` and TEXT content in `previewText` (`previewText = null` for AUDIO targets). GUIDANCE_QUESTION previews expose `type`, the snapshot `targetId` and immutable prompt text in `previewText`, with `senderId = null` and `messageType = null`. Replies never serialize the target's own reply metadata, so reply chains remain shallow. Bucket and object key are never returned. The URL is generated fresh from private S3-compatible storage when the response is serialized and must not be persisted by clients.
...
- `guidance.question.id` is the source/catalog question id. `guidance.question.instanceId` is the stable `conversation_prompt_snapshots.id` for this chat instance and is the value clients use for `replyTo.targetId` when quoting a guidance question. Legacy guidance rows without snapshots may return `instanceId = null`.
- A participant must reach `chat.first-chat.guidance.required-participation-score` since the current question was activated before requesting the current `progressionAction`. Ordinary authored TEXT characters count 1 point. Authored TEXT characters directly replying to the current guidance question snapshot count with `chat.first-chat.guidance.direct-question-reply-multiplier`, default 2. Quoted preview text contributes zero, AUDIO contributes zero, replies to partner messages count x1, and replies to historical guidance questions count x1.
- Advancement/completion requires both participants to independently request the current action. The API exposes only `myNextRequested`, `canRequestNext`, `completed`, current question, ordinal, `maxQuestions`, compatibility `requiredCharacters`, `requiredParticipationScore`, `directQuestionReplyMultiplier` and `progressionAction`; it does not expose partner readiness, partner request timestamp or partner score.
- Maximum questions per first chat is `chat.first-chat.guidance.max-questions`, default `3`. When both participants request from a non-final question, the next question activates and request timestamps reset. The final question remains active and incomplete; only after both participants reach score and request `COMPLETE` does the backend set `completedAt`, return `completed = true`, `canRequestNext = false`, `myNextRequested = false` and `progressionAction = null`.
- First-chat audio uses shared guidance progress: `answeredGuidanceQuestions = max(currentQuestionOrdinal - 1, 0)`. Local/test require `1`; dev/prod require `2`; `maxQuestions` remains `3`. Legacy first chats without guidance return `GUIDANCE_NOT_AVAILABLE`. Each participant may create at most one first-chat audio message; an exact idempotent replay of the same audio payload and direct reply target does not consume another slot.
...
- `POST /api/safety/reports`: create a user safety report and contain the current active interaction/opportunity between the reporter and reported user. Supported contexts are `CHAT`, `VISUAL_PROFILE`, `PERSONAL_MESSAGE` and `PROFILE_PHOTO`. The backend validates that the authenticated reporter and reported user are the two participants in the referenced chat or visual-phase match; `PROFILE_PHOTO` also requires the reported photo to belong to the matched partner. New reports return `201 Created`; duplicate `USER` reports for the same reporter, reported user, context type and context id return `409 Conflict` with `SAFETY_REPORT_ALREADY_EXISTS`. Containment uses `ChatEndReason.SAFETY_REPORT` when a chat is closed. Optional `blockUser: true` additionally creates or reuses a directional `UserBlock` from reporter to reported with `source = SAFETY_REPORT`; omitted or `false` does not create a permanent block.
- `CHILD_SAFETY_CONCERN` is accepted by direct user reports, chat safety cancellations and admin-created reports. It records a broad concern as a normal `PENDING` report; it does not establish a violation and does not automatically create a penalty, ban or permanent block unless `blockUser: true` is supplied on user-facing report requests.
...
- `POST /api/admin/safety-reports`: create an admin safety report. `contextType=USER` creates a general report about the reported user with `contextId = reportedUserId` and no match/chat context. Contextual admin reports can reference chat, visual profile, personal message or profile photo contexts. Admin-created reports are independent `ADMIN` reports that may coexist with a `USER` report for the same reporter, reported user, context type and context id. Admin-created reports do not auto-block, auto-close chats or auto-apply penalties; sanctions are applied only through explicit report confirmation.
...
Temporary penalties require positive `durationHours`; permanent penalties reject `durationHours` and have `expiresAt = null`. Confirming a safety report is the sanction point: report creation and dismissal do not ban or contain the reported user. Effective account bans block authenticated Reals access and defensively block matchmaking; creating a penalty removes the user from the queue if already queued. Multiple active rows are resolved as permanent winning over temporary, otherwise the latest temporary `expiresAt` wins. Permanent bans use full operational containment: active matches/chats/connections are closed with ban-specific terminal history, pending scheduling for contained connections is failed, affected engagement locks are released and Home is invalidated for affected users. Temporary bans use selective containment with `requiredUsableUntil = effectiveTemporaryBanExpiresAt + account.ban.temporary-resume-margin-minutes`; the default margin is 30 minutes and equality is preserve. First chats and active second chats always close with `USER_BANNED`; pending visual decisions close only when `requiredUsableUntil > VisualReview.expiresAt`, while already-decided banned participants preserve the visual interaction; scheduling states close only when `requiredUsableUntil > Connection.schedulingExpiresAt`; scheduled and available second chats use `confirmedDateTime + chat.second-chat.entry-window-minutes` as the deadline, not the two-hour chat timeout. Contained administrative closures remain reliability-neutral. The penalty expiration job only normalizes expired temporary rows to inactive; it is not the authority for access at the temporal boundary. Expiry of a temporary ban does not reopen engagements closed by containment.
...
`FAILED`/closed when it runs. When user reliability is enabled, scheduling
expiry records `SCHEDULING_EXPIRED_ACTION_PENDING` only for users with
current-round responsibility still pending: no current-round submission, or
current-round partner proposals still `PENDING` for them to resolve.
...
- `ACTIVE_PENALTY`: user cannot enter matchmaking while an effective account ban exists.
...
- `CHAT_MESSAGE_INVALID`: chat message input is malformed or invalid, including partial/malformed AUDIO reply multipart metadata.
- `CHAT_MESSAGE_REPLY_TARGET_NOT_AVAILABLE`: the requested direct reply target is unavailable, including missing/wrong-chat targets, own messages, unsupported message targets or invalid guidance snapshots. The error is intentionally opaque.
- `CHAT_MESSAGE_IDEMPOTENCY_CONFLICT`: the same `(chat, sender, clientMessageId)` was already used with a different normalized TEXT payload, accepted AUDIO SHA-256, direct reply target or message type.
...
- `FIRST_CHAT_APPROVAL_TOO_EARLY`: first-chat approval was attempted before the configured minimum elapsed time.
- `FIRST_CHAT_APPROVAL_PARTICIPATION_REQUIRED`: first-chat approval was attempted before both participants reached the configured confirmed-message threshold.
- `FIRST_CHAT_GUIDANCE_PARTICIPATION_REQUIRED`: the requester has not yet reached the required weighted participation score during the current first-chat guidance question interval.
...
- `FIRST_CHAT_GUIDANCE_COMPLETED`: first-chat guidance has already reached its persisted terminal completion state.
...
- `SCHEDULING_SLOT_CONFLICT`: a proposed or selected second-chat start is unavailable for scheduling, including conflicts with another confirmed second chat for the same user or an effective account-ban state that makes the commitment non-viable.
