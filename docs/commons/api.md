...
Audio messages are part of the same ordered chat-message stream as text, not a separate feed. Message DTOs add nullable `clientMessageId`, `messageType` (`TEXT` or `AUDIO`), `audio`, nullable `reactionType` (`HEART` or `null`) and nullable `replyTo`. Text messages keep non-null `content` and `audio = null`; audio messages have `content = null` and `audio = { url, durationMillis, contentType, sizeBytes }`. `replyTo` is a server-derived shallow preview of exactly one direct target; it never includes nested quote chains, target reactions or full target DTOs. MESSAGE previews expose target id, sender id, target message type and TEXT content (`previewText = null` for AUDIO). GUIDANCE_QUESTION previews expose the snapshot id and immutable prompt text with no sender/message type. Bucket and object key are never returned. The URL is generated fresh from private S3-compatible storage when the response is serialized and must not be persisted by clients.
