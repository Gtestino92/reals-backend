...
- `UserBlock` records directional permanent blocks. Matchmaking treats a block in either direction between two users as a bidirectional exclusion.
...
- `PushNotificationDelivery` deduplicates external push attempts per user, notification type and aggregate id. For `MATCH_FOUND` and `MATCH_FOUND_INVALIDATED`, the aggregate id is the match id. For `VISUAL_REVIEW_REMINDER`, the aggregate id is the match id; historical `VISUAL_REVIEW_AVAILABLE` rows also use match id and remain readable. For `MATCHMAKING_AVAILABLE`, the aggregate id is the matchmaking availability notification episode id. For grouped `SCHEDULING_AVAILABLE`, the aggregate id is deterministic from notification type, user id and the sorted activated connection ids from one activation job execution. For `SCHEDULING_PROPOSALS_RECEIVED`, the aggregate id is deterministic from notification type, connection id and round number, allowing one partner notification per round. For `SCHEDULING_CONFIRMED`, the aggregate id is the connection id. For `SECOND_CHAT_REMINDER`, the aggregate id is a deterministic reminder key derived from connection id and `minutesBefore`, so multiple configured reminder lead times can be sent once each. For `SECOND_CHAT_STARTED`, the aggregate id is deterministic from `second-chat-started:<connectionId>`. Delivery status includes sent, skipped-token, `SKIPPED_USER_PREFERENCE` and provider failure outcomes.
...
- Safety cancellation accepts optional `blockUser: true` to additionally create or reuse a directional `UserBlock` from reporter to reported with `source = SAFETY_REPORT` and `sourceReportId` pointing at the created report. Omitted or `false` does not create a permanent block.
- `CHILD_SAFETY_CONCERN` is a broad reported concern, not a confirmed finding. Direct reports and chat safety cancellations using it create a normal `PENDING` report and retain the same safety-report containment behavior as other user-created reports.
...
- General user safety reports outside chat cancellation validate a real chat or visual-review interaction and contain the current active pair interaction/opportunity. Chat containment uses `ChatEndReason.SAFETY_REPORT`. Duplicate `USER` reports for the same reporter, reported user, context type and context id are rejected with `SAFETY_REPORT_ALREADY_EXISTS`.
- Admin-created safety reports are independent `ADMIN` moderation artifacts. They may coexist with a `USER` report for the same reporter, reported user, context type and context id, and they do not block users, contain engagements, alter reliability or apply penalties until an admin explicitly confirms the report with a sanction.
- User-created reports always have `source = USER`, a non-null reporter and create or reuse a directional block from reporter to reported only when the user-facing request includes `blockUser: true`.
...
- Confirmed permanent bans operationally contain existing engagements in the same transaction as penalty creation. Active first-chat matches move to `CHAT_REJECTED`, active visual matches move to `VISUAL_REJECTED`, visible chats become `CANCELLED / USER_BANNED`, active connections become `CLOSED`, pending scheduling negotiations for contained connections become `FAILED`, and affected engagement locks are released. Account deletion uses the same full-containment mechanics but records `USER_DELETED`.
- Confirmed temporary bans block account access while effective and always remove the user from matchmaking, but containment is selective. The backend calculates `requiredUsableUntil = effectiveTemporaryBanExpiresAt + account.ban.temporary-resume-margin-minutes`; the default margin is 30 minutes, and equality belongs to the preserve side. First chats and active second chats always close. Visual interactions are preserved if the banned participant already decided, or if a pending decision still has `requiredUsableUntil <= VisualReview.expiresAt`. Scheduling pending/phase connections are preserved when `requiredUsableUntil <= Connection.schedulingExpiresAt`. Scheduled and available second chats are preserved when `requiredUsableUntil <= confirmedDateTime + chat.second-chat.entry-window-minutes`.
- Scheduling confirmation cannot create a second-chat commitment that is already non-viable for either participant under an effective account ban. Permanent bans make a slot non-confirmable; temporary bans require the same effective expiry plus resume-margin check against the candidate start plus the configured entry window. Rejections use opaque scheduling availability errors and do not expose another user's ban details.
...
- Temporary-ban expiry does not restore contained engagements. At `now >= expiresAt`, the account may authenticate and enter new matchmaking if otherwise eligible, preserved engagements remain in their existing lifecycle, and old matches, chats, connections and negotiations that were closed remain terminal.
...
- Outside `prod`, the temporary MVP photo shortcut returns `isPersonPhoto=true`, `isFullBody=true` and `validationStatus=VALIDATED` when provider `none` is used. In `prod`, startup guardrails reject provider `none`; if bypassed, the defensive no-provider path leaves technically valid photos as `validationStatus=PENDING`, `isPersonPhoto=false` and `isFullBody=false`.
- Profile photo provider `sightengine` is enabled in `dev` only by explicit smoke-test opt-in and is required in `prod`. DEV defaults to provider `none` and does not call Sightengine unless `PROFILE_PHOTO_MODERATION_PROVIDER=sightengine` is configured with usable credentials and endpoint. Provider `sightengine` performs one synchronous Sightengine multipart request per upload/replacement after technical validation and server normalization. That single request uploads the same normalized JPEG bytes that object storage receives and asks for `face-analysis`, `nudity-2.1`, `violence`, `gore-2.0` and `offensive-2.0`; provider output is mapped to internal Reals signal models and is not persisted.
...
- Profile photo content moderation is separate and persisted as `moderationStatus`. With provider `none` outside `prod`, the MVP compatibility path returns `APPROVED` without external review. With provider `none` in `prod`, startup guardrails reject the configuration. With provider `sightengine` in `dev` or `prod`, Reals policy evaluates provider-neutral sexual explicit, sexual suggestive, violence/threat, gore and hate/extremism scores. Sexual suggestive considers Sightengine `very_suggestive`, `suggestive`, `visibly_undressed`, `lingerie`, `suggestive_pose`, `suggestive_focus`, `sextoy` and `nudity_art`; it deliberately does not use `mildly_suggestive` or ordinary classes such as `bikini`, `swimwear_one_piece`, `swimwear_male`, `male_chest`, `cleavage`, `miniskirt`, `minishort` and `male_underwear` as automatic review evidence. Reject thresholds win over review thresholds; otherwise the photo is `APPROVED`.
- Owner/profile-management reads include all persisted profile photos and their `moderationStatus`. Admin moderation reads include current `NEEDS_REVIEW` photos. External partner profile reads, including visual-profile responses, expose only `moderationStatus=APPROVED` profile photos and generate read URLs only for those included photos.
