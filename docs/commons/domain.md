...
A quoted TEXT message is exactly one conversational message: it updates `lastMessageAt`, updates `lastMessageSenderId` for second chats, counts in generic message counts and participant-message checks, cancels pending second-chat mutual-completion or partner-inactivity requests in the same pre-message path as text, and may contribute authored content points to first-chat guidance. The referenced target itself creates no activity. An audio message is conversational for the same lifecycle purposes, but audio does not contribute points toward first-chat guidance advancement; guidance score aggregation counts only authored `TEXT` rows with non-null content.
