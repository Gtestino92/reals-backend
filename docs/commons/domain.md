...
- Outside `prod`, the temporary MVP photo shortcut returns `isPersonPhoto=true`, `isFullBody=true` and `validationStatus=VALIDATED`. In `prod`, technical validation alone returns `isPersonPhoto=false`, `isFullBody=false` and `validationStatus=PENDING` until real semantic analysis exists.
- Profile photo content moderation is separate and persisted as `moderationStatus`. With provider `none` outside `prod`, the MVP compatibility path returns `APPROVED` without external review. With provider `none` in `prod`, uploads may proceed but moderation persists as `NEEDS_REVIEW`. Future moderation providers may handle adult/nudity/safety checks, person detection, full-body detection, face/person consistency and minor/age-risk signals.
...
- Admin photo moderation review supports only `NEEDS_REVIEW -> APPROVED` and `NEEDS_REVIEW -> REJECTED`. A successful manual moderation decision records `PHOTO_MODERATION_UPDATED` on `PROFILE_PHOTO` and does not change `validationStatus`, `isPersonPhoto` or `isFullBody`.
