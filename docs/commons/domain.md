...
- General user safety reports outside chat cancellation validate a real chat or visual-review interaction and create the same directional block without closing any chat. Duplicate `USER` reports for the same reporter, reported user, context type and context id are rejected with `SAFETY_REPORT_ALREADY_EXISTS`.
