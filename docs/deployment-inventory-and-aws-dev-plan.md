...
| Deployment | Manual `Deploy AWS Dev` workflow resolves an immutable `sha-<shortsha>` image, assumes AWS role through OIDC, sends the repository-owned script over SSM, and validates public readiness and ping. | `.github/workflows/deploy-aws-dev.yml`, `ops/aws/deploy-backend.sh`, `docs/aws-dev-deployment.md` | Automation ready after GitHub Environment and AWS role/host prerequisites are configured. | Real AWS OIDC role, EC2 Name tag, SSM-managed host, Nginx, Docker, and runtime env file remain manual setup. | Medium |
| Rollback | Runtime script captures the previous container image ID before replacement and automatically recreates it if new health checks fail. Explicit rollback can deploy an older `development` ancestor SHA. | `.github/workflows/deploy-aws-dev.yml`, `ops/aws/deploy-backend.sh`, `docs/aws-dev-deployment.md` | Application-image rollback automated for container health failures. | Flyway rollback still requires migration compatibility discipline, restore drill, or forward-fix image. | High |
...
Rollback:
- Automatic rollback recreates the previously captured local image ID when the new container fails readiness or ping. Explicit rollback deploys a provided full SHA that is an ancestor of current `development`.
