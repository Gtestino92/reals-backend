...
| Docker image | Multi-stage Temurin 21 JDK build, Temurin 21 JRE runtime, non-root `reals` user, exposes `8080`, excludes `secrets`. CI builds with Buildx on GitHub-hosted `ubuntu-latest` and uses the GitHub Actions BuildKit cache; no explicit `--platform`, multi-platform build, or manifest list is configured. | `Dockerfile`, `.dockerignore`, `.github/workflows/ci.yml` | Ready to build for the runner platform. Treat current published images as `linux/amd64`. | Select x86_64 runtime initially, or add future multi-platform CI before using Graviton/ARM64. | Medium |
...
| `STORAGE_S3_BUCKET` | `src/main/resources/application-dev.yml` | Required application media bucket. New deployments must use this variable. Deprecated fallback: `S3_PROFILE_PHOTOS_BUCKET`. | Sensitive identifier | None | SSM Parameter | S3 client exists but operations fail or config binding fails | Backend |
